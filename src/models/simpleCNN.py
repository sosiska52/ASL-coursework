import torch
import torch.nn as nn
from torch.utils.data import DataLoader, Dataset, random_split
from app.visualizer import TrainingVisualizer

EPOCH = 10

# 1. Класс для предобработанных данных (оставляем без изменений)
class PreprocessedDataset(Dataset):
    def __init__(self, data_dict):
        self.data = data_dict['data']
        self.classes = data_dict['classes']
        self.class_to_idx = data_dict['class_to_idx']

    def __len__(self):
        return len(self.data)

    def __getitem__(self, idx):
        return self.data[idx]


# 2. Загрузка данных и разделение на train/val,
print("Loading data...")
PROCESSED_PATH = r'D:\GitHubRepo\ASL-coursework\src\data\processed\ASL_Alphabet_Dataset\asl_alphabet_train'
loaded_data = torch.load(PROCESSED_PATH)
full_dataset = PreprocessedDataset(loaded_data)
print("DONE")

# Разделение данных
train_size = int(0.8 * len(full_dataset))
val_size = len(full_dataset) - train_size
train_dataset, val_dataset = random_split(full_dataset, [train_size, val_size])

train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True)
val_loader = DataLoader(val_dataset, batch_size=32)

# 3. Инициализация визуализатора
visualizer = TrainingVisualizer()


# 4. Модель и оптимизатор (оставляем без изменений)
class SimpleASLNet(nn.Module):
    def __init__(self):
        super().__init__()
        self.layers = nn.Sequential(
            nn.Flatten(),
            nn.Linear(64 * 64 * 3, 128),
            nn.ReLU(),
            nn.Linear(128, len(full_dataset.classes)))

    def forward(self, x):
        return self.layers(x)


model = SimpleASLNet()
criterion = nn.CrossEntropyLoss()
optimizer = torch.optim.SGD(model.parameters(), lr=0.01)

# 5. Модифицированный цикл обучения
for epoch in range(EPOCH):
    # Обучение
    model.train()
    train_loss = 0.0
    correct_train = 0
    total_train = 0

    for images, labels in train_loader:
        outputs = model(images)
        loss = criterion(outputs, labels)

        optimizer.zero_grad()
        loss.backward()
        optimizer.step()

        train_loss += loss.item()
        _, predicted = torch.max(outputs.data, 1)
        total_train += labels.size(0)
        correct_train += (predicted == labels).sum().item()

    # Валидация
    model.eval()
    val_loss = 0.0
    correct_val = 0
    total_val = 0

    with torch.no_grad():
        for images, labels in val_loader:
            outputs = model(images)
            loss = criterion(outputs, labels)

            val_loss += loss.item()
            _, predicted = torch.max(outputs.data, 1)
            total_val += labels.size(0)
            correct_val += (predicted == labels).sum().item()

    # Расчет метрик
    avg_train_loss = train_loss / len(train_loader)
    avg_val_loss = val_loss / len(val_loader)
    train_acc = correct_train / total_train
    val_acc = correct_val / total_val

    # Обновление визуализатора
    visualizer.update(
        train_loss=avg_train_loss,
        val_loss=avg_val_loss,
        train_acc=train_acc,
        val_acc=val_acc
    )

    print(f'Epoch [{epoch + 1}/{EPOCH}]')
    print(f'Train Loss: {avg_train_loss:.4f} | Val Loss: {avg_val_loss:.4f}')
    print(f'Train Acc: {train_acc:.4f} | Val Acc: {val_acc:.4f}\n')

# 6. Визуализация и сохранение
visualizer.plot(save_path=r'D:\GitHubRepo\ASL-coursework\reports\figures\training_plot.png')
torch.save(model.state_dict(), r"D:\GitHubRepo\ASL-coursework\src\models\simple_asl_model.pth")